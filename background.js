...
/**
 * Show original layout (all workspace folders from bookspace, maintaining structure)
 */
async function showNoMatch() {
  if (isProcessing) {
    console.log('bookspace: Already processing, skipping...');
    return { success: false, message: 'Already processing' };
  }
  ...
  try {
    ...
    // Move all workspace folders from bookspace to toolbar (maintaining structure)
    const bookspaceChildren = await getFolderChildren(bookspaceFolder.id);
    let count = 0;
    
    for (const item of bookspaceChildren) {
      // Only move folders (workspace folders), not individual bookmarks
      if (item.type === 'folder') {
        try {
          await browser.bookmarks.move(item.id, {
            parentId: TOOLBAR_ID,
          ...
          count++;
          console.log(`bookspace: Displayed workspace folder "${item.title}" in toolbar`);
        } catch (error) {
          console.error(`bookspace: Error moving folder "${item.title}":`, error);
        }
      }
    }
    
    currentWorkspace = null;
    console.log(`bookspace: Showing original layout - ${count} workspace folders`);
    isProcessing = false;
    return { success: true, count };
    
  } catch (error) {
    console.error('bookspace: Error showing no-match layout:', error);
    isProcessing = false;
    return { success: false, error: error.message };
  }
}
